...
pub fn evaluate_expr(
    ...
    match expr {
        ...
        Expr::Call { args, caller } => evaluate_function_call(args.clone(), caller.clone(), env),
        Expr::Unary { operator, right } => {
            evaluate_unary_expr(operator.clone(), right.clone(), env)
        }
        Expr::BinaryExpr {
            left,
            operator,
            right,
        } => evaluate_binary_expr(left.clone(), operator.clone(), right.clone(), env),
        Expr::ComparisonLiteral {
            left,
            operator,
            right,
        } => evaluate_compare_expr(left.clone(), operator.clone(), right.clone(), env),
        Expr::ObjectLiteral { properties } => evaluate_object_expr(properties.clone(), env),
        Expr::AssignmentExpr { assignee, value } => {
            evaluate_assignment(assignee.clone(), value.clone(), env)
        }
    }
}
...
fn evaluate_assignment(
    ...
    match *assignee {
        ...
        Expr::Member {
            ...
            let _ = equate_member_expr(*object, *property, computed, *value.clone(), env);
...
fn equate_member_expr(
    ...
    let obj = evaluate_expr(object.clone(), env)?;
